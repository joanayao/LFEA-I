...
void log_ruido() {
    ...
    TH1F* histograma = new TH1F("histograma", "Uniformly Distributed Bins", 100, 0, 1023);
    TH1F* subtracao = new TH1F("histograma", "Uniformly Distributed Bins", 100, 0, 1023);
    ...
    std::ifstream file("RUIDO.CESIO.txt");
    if (!file.is_open()) {
    ...
    std::ifstream files("COM.FONTE.txt");
    if (!files.is_open()) {
