...
void chumbo1_fit() {
    ...
    //Leitura do Ficheiro
    std::ifstream file("chumbo1_sem_ruido.txt");
    if (!file.is_open()) {
    ...
    histogram->SetTitle("Pico de Absorcao Total Chumbo1"); // -> MEXER AQUI
    ...
    fitFunc->Draw("same");
    C.SaveAs("FIT_CHUMBO1.png"); // -> MEXER AQUI
    C.Update();
    C.WaitPrimitive();


    std::ofstream outputFile("Parâmetros_Pico_Absorção_Chumbo1.txt"); // -> MEXER AQUI
    if (!outputFile) {
