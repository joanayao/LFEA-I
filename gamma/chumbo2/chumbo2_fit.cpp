...
void chumbo2_fit() {
    ...
    //Leitura do Ficheiro
    std::ifstream file("chumbo2_sem_ruido.txt");
    if (!file.is_open()) {
    ...
    histogram->SetTitle("Pico de Absorcao Total Chumbo2"); // -> MEXER AQUI
    ...
    fitFunc->Draw("same");
    C.SaveAs("FIT_CHUMBO2.png"); // -> MEXER AQUI
    C.Update();
    C.WaitPrimitive();


    std::ofstream outputFile("Parâmetros_Pico_Absorção_Chumbo2.txt"); // -> MEXER AQUI
    if (!outputFile) {
